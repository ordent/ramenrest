...
class RestResponse
{
    ...
    public function successResponse($data, $cache = false)
    {
        if ($cache) {
            ...
            return response()->json($data, 200)->header('Cache-Control', 'max-age=0,must-revalidate');
        }
    }
    ...
    public function exceptionResponse($exception)
    {
        ...
        if (\App::environment() == 'local') {
            ...
            if (!config('trace_detail')) {
                $trace = json_decode(json_encode($trace));
            }
        }
        switch ($exception) {
    ...
    public function errorException($status = 500, $message = null, $detail = null, $exception = null)
    {
        $result = new \StdClass;
        $result->meta = new \StdClass;
        $result->data = null;
        $result->meta->status_code = $status;
        $detail = array_wrap($detail);
        if (!is_null($message)) {
